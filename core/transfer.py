...
def search_users_account_number(request):
    ...
    if query:
        account=account.filter(Q(account_number=query)|Q(account_id=query)).distinct()
    return render(request,"transfer/search-user-by-account-number.html",{'account':account,'query':query})
...
def AmountTransfer(request,account_number):
    ...
    return render(request,"transfer/amount-transfer.html",{'account':account})
...
def AmountTransferProcess(request, account_number):
    ...
    if request.method == "POST":
        ...
        try:
          ...
          messages.warning(request, "Enter Any Amount")
          return redirect("core:amount-transfer", account.account_number)  
    else:
